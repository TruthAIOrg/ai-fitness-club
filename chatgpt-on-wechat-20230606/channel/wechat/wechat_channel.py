...
class WechatChannel(ChatChannel):
    ...
    def send_the_group(self, reply: Reply):
        auto_timed_message_groups = conf().get("auto_timed_message_groups", [])
        logger.debug("[WX] send_the_group auto_timed_message_groups={}".format(auto_timed_message_groups))

        # 遍历群聊名称列表，对每个群聊名称调用 itchat.search_chatrooms
        for group_name in auto_timed_message_groups:
